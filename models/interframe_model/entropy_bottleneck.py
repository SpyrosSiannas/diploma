...
class EntropyBottleneck(nn.Module):
    ...
    def compress(self, inputs):
        ...
        symbols = symbols.reshape(-1, 1).repeat(
            1, values.shape[-1]
        )  # (num_symbols, channels)
...
if __name__ == "__main__":
    ...
    model.cuda()
    model.eval()
    x = torch.rand(3, 8).cuda()
    print("Running test...")
    out, likelihood = model(x)
    print(out.shape, likelihood.shape)
    strings, min_v, max_v = model.compress(x)
    print(strings, min_v.shape, max_v.shape)
