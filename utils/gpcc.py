...
def validate_tmc3():
    if not os.path.exists(os.path.join(tmc3_dir, "tmc3")):
        raise Exception(
            "tmc3 not found, please build it first via build.bash inside the root dir."
        )
    else:
        print("TMC3 found. Continuing...")


def gpcc_encode(filedir: str, bin_dir: str, show=False):
...
def gpcc_decode(bin_dir: str, rec_dir: str, show=False):
    validate_tmc3()
    subp = subprocess.Popen(
        tmc3_dir
        + "/tmc3"
